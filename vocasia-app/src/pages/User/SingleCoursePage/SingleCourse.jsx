    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                setEnrollmentData(getEnrollmentData);
                                ...
                                <div className="container">
                                    <div className="row justify-start marginCustom">
                                        <div className="col-xxl-8 col-xl-7 col-lg-8">
                                            <VideoPlayer course={enrollmentData.course} currentLesson={currentLesson}/>
                                        </div>
                                    </div>
                                </div>
                            </section>
                                ...
                                <div className="px-30 sm:px-20">
                                    <div className="accordion -block-2 text-left js-accordion mt-30">
                                        {lessonItems.map((item, index) => (
                                            <div
                                                className={`accordion__item ${activeItemId === item.id ? "is-active" : ""}`}
                                                key={index}
                                            >
                                                <div
                                                    onClick={() =>
                                                        setActiveItemId((prev) => (prev === item.id ? 0 : item.id))
                                                    }
                                                ...
                                                >
                                                    <div className="d-flex items-center">
                                                        ...
                                                        <span className="text-17 fw-500 text-dark-1">
                                                            [<small>{item.total_duration}</small>] {item.title}
                                                        </span>
                                                    </div>
                                                </div>
